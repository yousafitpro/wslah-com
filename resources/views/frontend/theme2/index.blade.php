...
@section('content')
    <section class="container">
        ...
        <div class="pb-14 md:pb-32 grid sm:grid-cols-2 md:grid-cols-3 gap-5 xl:gap-8" id="categories">
            @foreach ($food_categories as $category)
                <div class="bg-white dark:bg-secondary/50 rounded-xl shadow-shadowitem hover:shadow-shadowdark transition">
                    <a href="{{ route('restaurant.menu.item', ['restaurant' => $restaurant->id, 'food_category' => $category->id] + $append) }}"><img src="{{ $category->category_image_url }}" alt=""
                            class="w-full rounded-t-xl h-56 object-cover" /></a>
                    <div class="p-4">
        ...
        <div class="pb-12 md:pb-32 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 xl:gap-8 ">
            ...
            <p class="font-bold dark:text-white name truncate not_found custom" style="{{ count($foods) > 0 ? 'display:none;' : '' }}"> {{ __('system.messages.food_not_found') }}</p>
        </div>
